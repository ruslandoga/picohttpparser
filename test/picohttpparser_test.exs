defmodule PicoHTTPParserTest do
  ...
  test "it works" do
    packet = "GET / HTTP/1.1\r\nHost: example.com\r\n\r\n"
    assert PicoHTTPParser.parse(packet) == {:ok, "GET", "/", 1, [{"Host", "example.com"}]}
  end
  ...
  describe "ipv6" do
    test "with port" do
      ...
      assert PicoHTTPParser.parse(packet) ==
               {:ok, "GET",
                "http://[FEDC:BA98:7654:3210:FEDC:BA98:7654:3210]:4000/echo_components", 1,
                [{"host", "orange"}]}
    ...
    test "no port" do
      ...
      assert PicoHTTPParser.parse(packet) ==
               {:ok, "GET", "http://[FEDC:BA98:7654:3210:FEDC:BA98:7654:3210]/1234", 1,
                [{"host", "orange"}]}
    ...
    test "short ipv6 form" do
      ...
      assert PicoHTTPParser.parse(packet) ==
               {:ok, "GET", "http://[::1]/1234", 1, [{"host", "orange"}]}
